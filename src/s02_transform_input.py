...
def apply_all_col_dict(input_data: pd.DataFrame, monocore_df: pd.DataFrame, transform_columns: list) -> pd.DataFrame:
...
def create_evolution_dict(col_evolution: str) -> dict:
    ...
    for i in range(1, 10):
        ...
        evolution_dict.update(dict(zip(df_monocore["core_number_t"], df_monocore[col_evolution], strict=False)))
...
def expand_input_data(input_data: pd.DataFrame, evolution_dictionary: dict) -> pd.DataFrame:
    ...
    for i in range(69):
        filtered_keys = [key for key in evolution_dictionary if key.endswith(f"_{i}")]
