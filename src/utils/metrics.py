...
def std_absolute_error(y_true, y_pred) -> np.ndarray:
    """ Get standard deviation from absolute error """
    
    abs_error = absolute_error(y_true, y_pred)
    
    return np.std(abs_error, axis=0)
...
def mean_relative_error(y_true, y_pred) -> np.float:
...
def std_relative_error(y_true, y_pred) -> np.ndarray:
    """ Get standard deviation from relative error """
    
    rel_error = relative_error(y_true, y_pred)
    
    return np.std(rel_error, axis=0)